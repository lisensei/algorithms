...
for runs in range(30):
  ...
  sequence_result = "/gdrive/My Drive/CL/"+hp.sequence_name+"/summary of sequence " + str(hp.sequence_name) + ".csv"
  ...
  for epoch in range(hp.epoches):
      ...
      plt.imshow(processor.train_set.dataset[0][0][0])
      plt.show()
      fig = visualizer.plotMatrix([metrics["train_confusion_matrix"], metrics["test_confusion_matrix"]],show_figure=False)
