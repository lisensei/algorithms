...
class CNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.covs = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(num_features=55),
            nn.MaxPool2d(kernel_size=2, stride=2),
    ...
    def forward(self, x):
        out = self.covs.forward(x).reshape(len(x), -1)
        out = self.fc(out)
        out = self.rl(out)
        out = self.fcf(out)
        return out
...
for runs in range(10):
    ...
    log_path = "results/experiment_cifar10cnn/runs/sequence " + str(hp.sequence_name) + "/sequence " + str(
    ...
    filename = "results/experiment_cifar10cnn/metrics/sequence " + str(hp.sequence_name) + "/sequence " + str(
        hp.sequence_name) + "_" + "run " + str(
        hp.run_name) + "_" + postfix
    sequence_result = "results/experiment_cifar10cnn/metrics/sequence " + str(
